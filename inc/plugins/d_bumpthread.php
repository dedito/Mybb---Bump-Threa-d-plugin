...
function d_bumpthread_activate()
{
	...
	// add settings
	$disporder = $db->fetch_field($db->simple_select('settinggroups', 'MAX(disporder) AS disporder'), 'disporder', 'description') + 1;
	$settinggroup = [
		'name' => 'd_bumpthread',
		'title' => $db->escape_string($lang->bumpthread_settinggroups_title),
		'disporder' => $disporder,
		'description' => 'Podbij Temat - Ustawienia opcji' ,
	];
...
{
	global $mybb, $thread, $bumpthread, $templates, $tid, $lang;
	$lang->load('d_bumpthread');
